...
MAVSimAnimation::MAVSimAnimation(int width, int height){
	...
	m_camera = {0};
	m_camera.position = (Vector3){-20.0f,20.0f,0.0f};
	m_camera.target = (Vector3){0.0f, 0.0f, 0.0f};
	m_camera.up = (Vector3){0.0f, 1.0f, 0.0f};
	m_camera.fovy = 45.0f;
	m_camera.projection = CAMERA_PERSPECTIVE;

	m_mav_model = LoadModel("./resources/MAVModel.obj");
...
void MAVSimAnimation::render_step(Vector3 mav_pos, Vector3 mav_rot){
	...
	DrawModel(m_mav_model, mav_pos, 0.25f, WHITE);
	DrawModelWires(m_mav_model, mav_pos, 0.25f, RED);
	DrawGrid(100, 1.0f);
